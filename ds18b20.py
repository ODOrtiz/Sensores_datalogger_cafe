...
class DS18B20:
	...
	# ==============================================
	# Read temperature by device id
	# ==============================================
	def _read_temp_id(self, path):
		f = open(path,'r')
		lines = f.readlines()
		f.close()
		return lines
	...
	# ==============================================
	# Leer temperatura por id de sensor 
	# ==============================================		
	def getTemperatureById(self, path):
		lines = self._read_temp_id(path)
		retries = 5
		while (lines[0].strip()[-3:] != 'YES') and (retries > 0):
			# read failed so try again
			time.sleep(0.1)
